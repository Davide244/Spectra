...
public sealed class ProjectTests
{
    ...
    {
        // 'packs' is specified and there is no pack system, so decoding it would
        // produce a value that means nothing. Same three-tier rule the map uses.
        SpectraProject project = ProjectReader.Read(Utf8(Canonical));

        project.Unknown.Count.ShouldBe(1);
        project.Unknown[0].Name.ShouldBe("packs");
    }
