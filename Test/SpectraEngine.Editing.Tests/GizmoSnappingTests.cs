...
/// is felt through a drag: the <em>result</em> is what lands on the grid, an
/// off-grid selection is pulled onto it rather than carrying its offset along,
/// and the axes a handle does not free are never quantized behind the user's
/// back.
/// </summary>
public sealed class GizmoSnappingTests
{
    ...
    [Fact]
    public void Snapping_pulls_an_off_grid_selection_onto_the_grid()
    {
        // This is the test that distinguishes snapping the RESULT from snapping
        // the DELTA. A node at 0.3 dragged by 3.9 lands at 4.2 unsnapped; the
        // result rule rounds that to 4.0, while rounding the delta instead
        // would give 0.3 + 4 = 4.3 and leave the node off-grid forever.
        var harness = GizmoHarness.ThreeQuarterView();
    ...
    [Fact]
    public void A_snapped_plane_drag_lands_on_the_grid_in_both_of_its_axes()
    {
        var harness = GizmoHarness.ThreeQuarterView();
        ...
        node.LocalPosition.X.ShouldBe(4f, RoundTrip);
        node.LocalPosition.Y.ShouldBe(6f, RoundTrip);
    ...
    public void The_grid_still_takes_effect_on_the_first_frame_the_cursor_actually_moves()
    {
        ...
        // of real movement and the destination is on the grid, offset and all.
        var harness = GizmoHarness.ThreeQuarterView();
        var start = new Vector3(3.7f, 2.2f, -0.4f);
        SceneNode node = harness.AddSelectedNode(start);
