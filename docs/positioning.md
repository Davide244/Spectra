...
1. **The license. DECIDED IN PRINCIPLE 2026-08-28: open source, engine under MPL-2.0, templates and assets under MIT, and the name held back by trademark.** The brief was "open source, not extremely copyleft, and I do not want people republishing my engine under a different name", and those two requirements pull apart into three mechanisms rather than one.

   - **The engine: MPL-2.0.** Copyleft at the level of the FILE, not the project, which is exactly the "not extremely" the brief asks for: a game that ships Spectra keeps every line of its own game code proprietary, and the only obligation is that changes to Spectra's own source files stay public under MPL. That is what makes the feared outcome impossible: a rebranded fork may call itself anything it likes, but it cannot be a CLOSED rebrand, because its engine changes have to be published with the original copyright notices intact. A permissive licence (MIT, Apache-2.0, BSD) permits the closed rebrand outright, and the strong copyleft licences (GPL, AGPL) would force every game built on the engine to be GPL, which would end the commercial-game audience the whole positioning is aimed at.
   - **Why not LGPL, which is the other "weak copyleft" answer:** LGPL's relinking obligation is written for dynamic linking, and this engine is NativeAOT. Everything is statically linked into one binary, so LGPL §4 would oblige every shipped game to distribute relinkable object files. MPL-2.0's "Larger Work" clause (§3.2) permits static linking under other terms outright. This is an engine-specific reason, not a preference.
   - **Templates, example projects and starter assets: MIT (or CC0 for pure art).** These exist to be copied into a user's game, and MPL is file-scoped, so a template file copied into a project would drag MPL with it. The one thing a starter template must never do is attach an obligation to the game it starts.
   - **The name: a trademark policy, not a licence clause.** No copyright licence can stop a fork from renaming itself, and none should. What protects "Spectra" is holding it as a mark and publishing a short `TRADEMARK.md`: forks must rename, and may say "based on Spectra". Apache-2.0 §6 spells this separation out explicitly and is worth reading as the model even though the engine is not using Apache.

   **Still owed:** the actual `LICENSE`, `TRADEMARK.md` and per-directory headers, and a lawyer's read before the first public release. Nothing above is legal advice, and the MPL-versus-Apache call is the one part worth a second opinion, because it trades some adoption for the anti-rebrand guarantee.
