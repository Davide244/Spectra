...
*The one real gap.* A **mesh node saves without its geometry**. `MeshRenderer` holds a `Mesh` and a `Material`, both live objects; `Mesh` carries no name, path or owning-asset reference (`Renderer.CreateMesh` takes raw spans, so nothing at creation time *could* record an origin), and `ModelInstantiator` writes only a name, a transform and the renderer onto each node. The node keeps its identity, name, placement and children, and `MapSaveReport` reports what was lost rather than letting it vanish. Closing it means recording `(model path, submesh index, import options)` on the node, and all three are needed: the index is positional into `ModelAsset.Meshes`, and the options reshape that list while being pinned to whichever caller loaded the path first. That is a scene-graph change with a design in front of it, not something to guess at inside a codec.
