  ...
  "nodes": [
    {
      ...
      "script": { "kind": "server", "source": ["local part = script.Parent",
...
**Unknown-member preservation, exactly.** At an unrecognised property name (and `"editor"` is reserved, never captured): record `long start = reader.TokenStartIndex`, call `reader.Skip()`, capture `utf8[(int)start .. (int)reader.BytesConsumed]` into an ordered per-node list; on write, replay with `WritePropertyName` + `WriteRawValue`. **Verified constraint that makes this work:** `WriteRawValue` documents that the writer's `Indented` and `Encoder` settings are *not* applied to raw content — it is emitted as-is. So a preserved value keeps the original file's indentation, which yields byte identity **only because a preserved member's nesting depth never changes**. That is a real invariant the reader must uphold, not an accident.
...
| **D12** | `.scmap` container v1 + `scook` map bake | L | D4, D10, D11 (`F1` landed) | **Supersedes and deletes `P11b`** |
