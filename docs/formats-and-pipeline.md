...
**"Zero CSG at load" is a MEASUREMENT, because every other way of asking would pass with a carve running behind it.** A re-carved world draws the same walls, twice, and a test that checked the picture, or the chunk count, or that the level looks right would be perfectly happy with it. So the carve counts itself: `Csg.CarveInvocationsOnThisThread` is incremented at the top of `CarveCore` (the full carve, once per compile) and of `CarveSingle` (the incremental re-carve, once per brush), a load is bracketed by it, and the delta is the claim. **It is thread-static rather than a plain static**, which is the difference between an oracle and a flaky test: this suite runs in parallel with every other one, so a process-wide counter would be moved by any unrelated compile on any thread and a real regression would be indistinguishable from an unlucky schedule. Carving happens on the render thread and on thread-pool workers, and a caller measures the thread it loads on, which is the only thread its own load could have carved on. **The number is also printed on every real load**, so the claim holds in a shipped build rather than only in a test.

**The double-geometry guard is a runtime REFUSAL that counts and names itself, not a convention.** While a `CompiledStaticWorld` is installed, `RebuildStaticWorld`, `RebuildStaticWorldIfDirty`, `ProcessStaticWorldCompilation` and all three automatic dirty marks stand down; `Scene.RefusedStaticWorldRebuilds` and `RefusedStaticWorldDirtyMarks` count them and `StaticWorldGuardMessage` says why. **Refusing rather than throwing** because the callers are load-time paths that already have a world on screen and a throw would turn a level somebody can see into a crash; **counted rather than silent** because a public method that quietly does nothing is the same defect the latched editor suspension once was. It is said out loud from `ProcessStaticWorldCompilation` and nowhere else, because that is the only static-world method that carries a logger and the marks it reports run from property setters on the editing hot path, where a line per call is a line per frame. The loader's own half is that **a node whose `PayloadKind` is `StaticWorldBrush` gets NO brush, whether or not `BRSH` carries its planes**: `ScmapBrushSource.IsReCarvable` is the only predicate it consults, and `CompiledMapLoadReport.BakedBrushSourcesSkipped` counts the planes it was offered and declined, which is what makes "the guard fired" a number rather than an absence.

**A `FlatBspTree` over a mapped view needed a `MemoryManager`, and the SAFE implementation is the one that re-asks.** `FlatBspTree` takes a `ReadOnlyMemory<FlatBspNode>` because the block "is a plain array today and a memory-mapped view of a cooked map later", and later is now; a span into a mapping is not a `Memory`, there is no `MemoryMarshal.Cast` for one, and copying every cell's nodes into a managed array spends at load exactly what the zero-copy layout exists to save, on the one structure a level keeps for its whole life. The obvious implementation pins a pointer once, and that is wrong twice over: a pointer into a released mapping is an access violation with no managed stack, and a pointer into a POOLED array, which is what a loose-file blob is, is collectable and movable and fails silently instead. `MappedBspNodes.GetSpan` re-reads `ContentBlob.Span` on every access, so a use-after-free is an `ObjectDisposedException` naming the type. **`Pin` is refused by name rather than implemented**, because nothing in the query path pins and a `MemoryHandle` would be a promise about an address this design deliberately does not make. `ScmapDocument` grew `ChunkBspBlobFileOffset` for the same reason: a span cannot say where it came from, and re-deriving one by differencing two spans' addresses would be a pointer comparison standing in for a number the reader already had.

**`StaticWorldChunkMesh` gained its own `Coord` and `RenderBounds` and its `Artifact` became NULLABLE, and the alternative was a lie in the data.** Every consumer that read `chunk.Artifact.RenderBounds` was asking a question about a GPU-resident chunk, not about the compile that happened to produce one, and an adopted chunk has no compile. Fabricating a `ChunkMesh` with empty `ChunkSubmesh` arrays to fill the field would have satisfied the type and told everything downstream that the cell draws nothing. The artifact is still there and still nullable-by-design: it is the swap path's change detector, `ReferenceEquals(existing.Artifact, artifact)`, and a compiled world is never recompiled, so there is nothing for it to detect. `StaticWorldSubmesh.Source` became `SourceMaterial` for the matching reason: the entry needs the interned id to re-resolve its material and never needed the CPU arrays, which on this path are a window into a file.

**The 7xxx verifier arm, and why a compiled map is the largest one.** `SC7008` is a compiled map naming an asset that is not in the pack it ships in and `SC7009` is a compiled map this engine's own reader refuses; both are `Fatal` in `CookGate`, because an unclassified code defaults to fatal and `CookGateTests` fails the run instead. The first is the failure §2.1's verifier exists for, one format over: an `ASTB` row is the only way a baked chunk names the material its surfaces wear, a row nothing cooked binds the engine's default material at runtime, and the cook structurally cannot see it because a map rule and a material rule each succeed against the author's folder while the entry one of them needed can still fail to reach the file. The second catches what the digest cannot: a level baked at another format version, another geometry version, another vertex layout or on another cell size hashes perfectly and is still refused on frame zero. **Every row resolves through the SAME redirection the engine uses** (`ModelContentPath.Resolve`, `ImageContentPath.Resolve`), or every model in every cooked pack would be reported missing; a kind this build has no cooked lane for resolves literally rather than being skipped, so a future row type is not silently unverified from the day it is written.
...
**What the runtime load does NOT do, named so each is a gap rather than a discovery.** A `MeshInstance` node is rebuilt in its place, with its id, drawing nothing, and its name goes in the report: the format gives it one asset row and no submesh index, so binding submesh 0 would be right for a single-submesh prop and quietly wrong for every other one. **A compiled map has no placement list, so `BrushPlaneCollisionSource` finds no plane sets and the character mover has nothing to walk on** - a compiled map is today a level you can look at and not one you can walk in, and closing that is `COLL`'s job rather than this loader's. `CompiledStaticWorld` answers `ContainsPoint` and not `Raycast`, because the DDA walk has no caller on this path yet and a second copy of it written for nobody is a second thing to keep in step. `Brush.Transform` is not in `BRSH` and is rebuilt as the identity, which is exact for every node-attached brush (the scene snapshots the node's world matrix into a placement instead) and lossy for a standalone one. And the lights, spawns, entities and scripts §2.7 already names as absent are absent here too, which is why `CompiledMapLoadReport.FormatGaps` is a constant printed at Warning on every load: a level that quietly loses its lights is exactly the failure a standing line exists to prevent, and no amount of looking at the file can tell a loader whether the author put one there.

**AS BUILT (2026-09-03), the bake oracle over a CORPUS, and the suite it now belongs to.** `Test/Spectra.Kitchen.Tests/BakeOracleTests.cs` is `P11b`'s replacement guard in the form this section specified it: cook through the real `MapRule`, load through `CompiledMapLoader`, and assert that the per-cell vertex arrays, index arrays, submesh directories **and flattened BSP nodes** are element-identical to a fresh cache-free `CsgWorld.Build` plus `BspFlattener.Flatten` of the same bundle. `ROADMAP.md` `P11b` now names the file, so the cross-reference resolves to something that exists.

- **The corpus is the deliverable, because one map cannot fail in four ways.** D-Stage 21 landed a single instance of this claim over the flush-coplanar room, and that room writes ONE submesh directory of two entries, one cell that owns geometry, and no cell carrying a tree without one - three quarters of the format's shapes are simply never emitted by it. `BakeCorpus` adds three more: **`cavities`**, four subtractive shapes (a tunnel open at both ends, a **sealed pocket** touching no face of the solid it is inside, a cut straddling the x = 0 cell plane, an aperture through a second brush), because a negative emits no skin of its own and its whole contribution is cavity walls seeded into the brush it cuts; **`palette`**, one cell wearing SEVEN materials with six of them interned in the REVERSE of the order the node walk references them, so the file's ascending-asset-row order is a real permutation of the compile's ascending-material-id order rather than the identity, plus a brush wearing none so `NoAssetIndex` sits in a directory beside real rows; and **`sprawl`**, thirty-two cells with a slab spanning five of them, a brush straddling a cell face, a brush straddling a cell corner on all three axes, an island a hundred units out and one brush rotated off every axis. Measured across the four: **49 cells against the single room's 8, 619 BSP nodes against 84, and 38 cells carrying a tree with no mesh at all**, which is the shape a single-cell map structurally cannot have.
- **Element-identical means BIT-identical, and the two float values that make that a real distinction are `0f` and `-0f`.** They compare equal and are different numbers; the compile cache keys on exact equality and the determinism oracles compare bytes, so a comparison here that forgave a last-place difference would forgive precisely the class of bug the oracle exists for. Planes, normals, plane offsets, render bounds and every vertex go through `BitConverter.SingleToInt32Bits`.
- **Stated against what the RUNTIME received rather than against what the file holds.** `MapRuleTests` already asserts the file's own side over one room; the load adds the `ASTB` remap, the spans handed to `CreateMesh` and the `FlatBspTree` built over the mapped bytes, and each is a place the bake's arrays can arrive intact and be delivered wrongly. The BSP arm is new either way: nothing anywhere compared flattened nodes, and `CompiledMapLoadTests` asks four point queries, which a plane an ulp out of place will not move.
- **Each fixture names what it alone reaches, and a test MEASURES that rather than trusting the comment.** `Every_fixture_exercises_what_it_claims` reads the properties off the compile: `palette` must produce a seven-entry directory whose two orders really disagree, `sprawl` must produce twelve or more cells, a cell with a tree and no mesh, a split plane off every axis, and a widest directory of exactly ONE (it is the control that proves the corpus is not four copies of one test), and `cavities` must show that the cuts CUT - triangle counts differing from the same level with every negative deleted, plus the sealed pocket's middle empty in the carved world and solid in the uncut one. A fixture that has quietly stopped biting reports coverage that does not exist, which is worse than one that was never written.
- **Falsified three ways, and the three signatures differ, which is the evidence the corpus is not redundant.** *(1) One plane offset out by a millimetre in the flattened tree*: caught by the BSP arm on **all four** fixtures - and by **nothing else in the repository**, the other 315 tests staying green, which is the coverage hole this stage closes. *(2) Each submesh paired with its neighbour's geometry, ascending asset order preserved*: caught by `flush-coplanar-room`, `cavities` and `palette`, and **not** by `sprawl`, whose one material per cell makes the rotation the identity. *(3) A cell with a tree and no mesh losing its `CBSP` blob*: caught by `flush-coplanar-room`, `cavities` and `sprawl`, and **not** by `palette`, whose single cell has both.
- **No real defect was found, which is stated plainly rather than left as a green run.** The bake, the file and the load agree bit for bit across every fixture, including the rotated brush whose vertices are the output of a quaternion composed into a world matrix rather than numbers somebody could have typed. What the corpus did find is the blindness above: a millimetre in a baked split plane passed every test this repo had.
- **The oracles are a NAMED SUITE now, and the reason is reporting rather than coverage.** `[Trait("Suite", "Determinism")]` marks `CookDeterminismTests`, `ScmapDeterminismTests` and `BakeOracleTests`; `dotnet run --project Test/Spectra.Kitchen.Tests -- -trait "Suite=Determinism"` runs the 33 of them, and the `determinism` CI job runs it on **both hosts**. Every one also runs inside the ordinary suite in two other jobs, so nothing new is checked; what is new is that a failure arrives as "Cook determinism" rather than as one red name among 340, and these fail for a reason nothing else in the repo fails for. The marker is on the CLASS so a new oracle joins by being written rather than by somebody remembering to list it. **The job fails on a SKIP as well as on a failure**: the two-process oracles skip themselves when `scook` is not beside the test binary, and a green run over four unrun oracles is exactly the silently-weakened gate this workflow refuses everywhere else.
