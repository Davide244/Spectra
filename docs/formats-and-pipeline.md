...
**The two-process oracle re-enters the TEST binary, and that is temporary.** .NET randomises the string hash seed per process, so an in-process comparison structurally cannot detect a hash-order dependency, which is exactly why §4.3's cook oracles drive the real `scook` through `Process.Start`. The compiled-map writer has no CLI verb until the map bake gives the cook a map to bake, so the child is this same test binary, re-entered through an environment variable and a `[ModuleInitializer]` that writes the fixture and exits before a test is discovered. Each child also writes its own string hash codes beside its map and the two are required to **disagree**, because byte identity across two processes proves nothing unless the two really did hash differently. Falsified deliberately against a build that emitted the string table in hash order: two of the three determinism tests and four of the eighteen builder tests go red. Once `scook` bakes maps, the ordinary clean-cook oracle covers this file and the harness can go.

**What is not built, named so it is a gap rather than a discovery.** `CMSH` and `CBSP` are claimed and empty, so `ScmapBuilder` **refuses** a directory entry declaring a non-zero mesh or BSP blob rather than writing a directory that points at nothing: a cell whose geometry silently does not exist renders as a hole in the world. `BRSH`, `NBND`, `ENTT`/`ECON` and the three script sections have no producer. `PayloadFlags` bits 3 to 6 are written as `Inherit` because the engine has no realm or state enum yet, and the format owns that numbering, so the enum that lands later must match it. And `PackVerifier` has no 7xxx arm yet, so nothing cross-checks a compiled map's `ASTB` against the pack it ships in.
