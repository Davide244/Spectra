...
**Therefore the compensation belongs in the V axis of UV generation, and can never belong per backend at upload.** Cooked BC data arrives top-down, which inverts the meaning of v relative to the loose-PNG path; because no backend disagrees, exactly one flip has to move and it must move identically everywhere. The migration is: delete `ImageDecoder.FlipRowsInPlace`, adopt v = 0 as the TOP of the picture (which is also KTX2's, DDS's and glTF's convention, so cooked data and every imported model then agree with the engine for free), and flip V once where UVs are generated: `FaceSurface`'s V axis and `ChunkMeshBuilder` for brush faces, and the model importer's UV0 for meshes. It is a content-visible change and belongs with `D6`, not before it; the value of settling it now is that the alternative, two conventions distinguished per texture threaded to every sampler, is ruled out, and so is any per-backend branch.
...
Updated state (D-Stage 26): `Audio/` is real. There is a device, a context, a listener, a 32-source pool with an oldest-finished reclaim, `StreamingVoice` over a refilled buffer queue, and a disabled mode for a machine with no sound card. There is still **no mixer**: no buses, no submix, no DSP, no attenuation curves beyond OpenAL's own distance model. **The format is still the small half of the milestone**, and over-specifying it now means specifying it wrong in exactly the way a format cannot be fixed once content exists. What the runtime already settled is the field this header cares most about: **loop points are sample frames, and the runtime plays them by buffer-queue arithmetic rather than by `AL_LOOPING`** (`AudioLoopCursor`), so `LoopStart`/`LoopEnd` below are consumed exactly as written with no sub-buffer restriction, and the last paragraph of this section is no longer a constraint to design around.
...
*Not built here.* Nothing about packs, cooking, `.vscode/` or `spectra.d.luau`: all three depend on arcs that do not exist yet, and `.vscode/` in particular would ship a config pointing at a generated file nobody generates.
