...
- **There is no way to make a brush inert.** `Scene.SnapshotFullWalk` (`SpectraEngine.Core/Scene/Scene.cs:1349–1369`) iterates `Nodes` and admits **every** node carrying a `Brush`, unconditionally — there is no enable flag anywhere on that path. `SceneBvh.IsSpatial` (`SpectraEngine.Core/Scene/SceneBvh.cs:146`) is `node.MeshRenderer is not null || node.Brush is not null` — ancestry-blind. *(Still true of the tree today. And note that `BrushKind` — [`physics.md`](physics.md) §2.3a, milestone `P7a` — does **not** close it and is not a substitute for `NodeState`: a `Part` brush leaves the **carve**, but it still renders, still sits in the BVH, still picks and still collides. `Dormant` is the axis that makes a node contribute nothing to anything. The two are orthogonal and both are needed.)*
...
> **Reading these sketches after `P7a`.** Every `_subtreeBrushCount` above becomes the **total** projection of the two-lane field ([`physics.md`](physics.md) §2.3a), and the refusals keep their exact meaning — see the counter correction under R15. But the two elided `/* ...unchanged... */` blocks are **not** unchanged: `physics.md` §2.3a gates 7 and 8 split them, so the counter **adjustment moves both lanes** while the `MarkStaticWorldDirty()` beside it fires **only when the moved child's WORLD lane is non-zero**. Reparenting a subtree of part brushes must dirty nothing. That is a change to `AddChild`/`RemoveChild` and it belongs to `P7a`, not to this rule — the sketches here are about realm and state, and are elided precisely where they are.
...
#### The counter split, corrected — and why `BrushKind` adds ZERO new subtree invariants

This rule previously said: *"When `P7` splits the brush counter, migrate all three checks to `IsStaticWorldBrush` / `SubtreeStaticWorldBrushCount` in the same commit."* **Both halves of that instruction are now wrong, and the correction makes R15 safer rather than weaker.**

- **There is no "split" that produces a second counter.** [`docs/physics.md`](physics.md) §2.3a ships `_subtreeBrushCounts` as **one `long` field holding two 32-bit lanes** — total in the high half, static-world in the low half — mutated by exactly **one** private ancestor-chain writer, and exposed as two read-only projections (`SubtreeBrushCount`, unchanged in meaning and unchanged for every existing caller, and `SubtreeStaticWorldBrushCount`). **This rule's "do not add a third subtree invariant" is honoured by construction rather than by discipline**: two numbers that cannot be updated independently are one invariant, and a randomized attach/detach/reparent/kind-flip test recounts both lanes recursively to pin it. The counter-count this document is protecting does not change.
- **R15's three sites migrate to the TOTAL lane, not the world lane.** Reasoning: the world lane would let a subtree of *part* brushes slide under a `Server` node, and — until the relaxation below actually ships — that is exactly the divergence R15 exists to refuse. Migrating to the total lane is **behaviour-preserving**: every brush stays permanently `Shared`, the refusal message is unchanged, and pin 3 keeps passing without being re-baselined. (It is also what `ScaleGizmo` reads for its own unrelated refusal, so one lane serves both and neither drifts.)

#### The relaxation: R15 migrates by COLLISION, not by carve — and it does NOT ship with `P7a`

The tempting reading of `BrushKind` is *"the second paragraph below only argues about brushes that carve, so a part brush may live anywhere."* **That answers one of R15's two arguments and leaves the other standing.** The imprint argument (a server-only brush publishes its shape as the hole it leaves) genuinely does not apply to a part, which carves nothing. The **collision** argument does, verbatim: `IServerAuthority.Validate` ([`networking.md`](networking.md) §4.4) is a clamp plus a solidity test against the world the authority holds, and [`physics.md`](physics.md) §3.1 makes the divergence unrecoverable — **the client calls `b3World_Step` zero times and predicts its mover against its own static hulls, so it cannot discover a server-only solid.** The mirror case is equally broken with the sign flipped: a *client-only* solid blocks a player where the authority believes the space free, so no correction ever fires and the server's model of that player is simply wrong.

**Final rule.** A non-`Shared` node may carry a brush only when **`BrushKind == Part`** *and* that part carries **no collision the other side's predicted mover must agree with** — `CanCollide == false` sensors and triggers, and render-only geometry. A **collision-bearing** part brush stays `Shared`, refused with a message naming `CanCollide = false` as the route, alongside this rule's existing `State = Dormant` and `MeshRenderer` routes.

**Sequencing, and it is not optional.** `PhysicsFlags` does not exist, so `CanCollide` does not exist, so the predicate above cannot be evaluated. Therefore:

- **At `P7a`** (the milestone that ships `BrushKind`): the three refusal sites move to the **total** lane. Behaviour is identical to today — every brush of either kind is permanently `Shared` — and the only change is which projection is read.
- **The relaxation lands with the milestone that first ships `CanCollide`** (`physics.md` `Y0`/`Y3`, or `Y8` for the touch half), never before. **`P7a` must not advertise the server-side clip volume**, and neither may anything downstream of it. R16 is untouched: geometry secrecy is still not offered.
...
**One line of that message becomes a lie the moment `BrushKind` ships, and must be corrected in the same commit.** A `P8` trigger volume is a **`BrushKind.Part` brush** ([`physics.md`](physics.md) §4.3), and at `P7a` a part brush is still refused on a non-`Shared` node by the total-lane migration above — so *"for a server-side volume, use a trigger/query volume"* would name a route the code refuses. Until the relaxation ships, the third sentence reads: *"A server-side volume is not available yet: it needs `CanCollide = false`, which arrives with `Y0`."* When the relaxation lands, it becomes: *"For a server-side volume, use a Part brush with `CanCollide = false`, which never enters the carve and which no predicted mover consults."* The message must never name a route that throws.
...
**`BrushKind` is the first such predicate to actually ship, and it obeys this rule exactly** ([`physics.md`](physics.md) §2.3a). The `BrushKind` setter is the **one** admission write in that design: it early-outs on an equal write, and on a real change it moves ±1 between the two counter lanes and calls `Scene.MarkAdmissionChanged()` — **both conditional on the node actually carrying a brush**, since a kind flip on a brushless node changes nothing admitted and must signal nothing. A kind flip is a count-preserving admission change of precisely the shape this rule was written about (node A leaves the placement list, node B could join it in the same frame, count identical, slot mapping shifted), so `MarkAdmissionChanged` is not belt-and-braces there — it is the whole guarantee. Note also that **no other write on a part-brush node may bump either version at all**: `physics.md` §2.3a states that as a checkable invariant with three pinning tests, and one of the three fails against unmodified code today.
...
| Roblox | Verified semantics | Spectra | Status |
| --- | --- | --- | --- |
| `Lighting` | global lighting properties **and** a container for `Sky`/`Atmosphere`/post-effects | **Three things, three homes.** Global properties → a typed `Scene.Environment` settings struct; sky/atmosphere → asset references on it; post-effects → an ordered chain owned by the render arc. **Lights are spatial `SceneNode`s with a `Light` payload.** This overturned an earlier `roblox-to-spectra.md` row ("a `Scene.Lighting` node with attributes") — a node implies a transform, a parent, a realm and a subtree brush count, none of which mean anything for fog density. **That correction has landed**; the row there is marked *Corrected* and agrees with this one. **AMENDED 2026-08-29 by `ROADMAP.md` `R19b`:** the ruling above governs **global** fog and does not reach a **bounded fog zone**, for which a transform and a parent mean exactly what they always mean. The split is therefore global defaults on `Scene.Environment` and spatial overrides as a `FogVolume` payload on a `SceneNode`, blended by priority with a soft-edge distance. Recorded here as well as in the roadmap, because a silent exception to a stated ruling is how the next reader derives the opposite rule | **planned** |
...
13. **`BrushKind` gets the same treatment as realm, and two of its affordances are NOT part of this list — they ship earlier, without a UI shell.** [`physics.md`](physics.md) §2.3a adds a second declared, per-node, admission-relevant bit (`World` / `Part`), and every argument on this list applies to it: an unmarked part brush that renders but never carves is indistinguishable from a CSG bug, which is item 3's argument and commit `d4701d6`'s lesson. But the split ships at **`P7a`**, long before the Uno shell exists, so the affordances divide:
    - **Shipping WITH `P7a`, because they need only machinery that already runs on all three backends** (and they are acceptance criteria there, not here): part-brush outlines drawn **always** through the existing depth-off `DebugDraw` line pass, and `WorldBrushes: N  PartBrushes: M` in the periodic stats line.
    - **Belonging to this list, and blocked on the same UI shell everything else here is blocked on:** a gutter badge or column glyph distinguishing part from world (fill or glyph, never colour alone, per item 2); the `View as` lens gaining a *"parts only / world only"* filter fed by the **same** predicate the runtime uses (item 4); the right-click → Brush kind → … gesture with illegal options **disabled carrying their refusal text** (item 6) — a node with a live body shows `World` disabled; and the **convert dialog**, which must state the two disclosures `physics.md` §2.3a puts in `ConvertBrushKindCommand`'s undo label: the UV bake is mandatory, and the world-aligned face flag does not survive a round trip. Item 8 applies unchanged — conversion is an `IEditorCommand` addressed by `SceneNode.Id` recording absolute before/after values, batched into one `CompositeCommand` for a multi-selection so a bulk convert is **one** undo entry.
    - **Item 10 does NOT apply**, and that is the point of the bit being non-inherited: there is no illegal drop and no drag consequence to preview, because **`BrushKind` never changes on a reparent**.
...
Do not claim "smaller". The honest count for this model is: realm as an inherited enum, the narrowing rule, what a reparent does, `Shared`-scripts-run-on-server, per-property realm and where it does *not* apply, brushes-must-be-`Shared`-and-why, modules instance per side, `Dormant`, spawn rules, `JoinPriority`. That is about ten rules, **three of which are exceptions** — and exceptions are precisely what made Roblox's model expensive. **`BrushKind` adds an eleventh** (a brush is world geometry or a part; parts do not carve, do not fuse and are not in `CsgWorld`'s queries) — but it is the cheapest kind to add, because it is **not inherited**: it has no narrowing rule, no reparent semantics and no effective-versus-declared distinction to teach, which is three of the four hard things about the other ten. Meanwhile a Roblox developer does not *experience* containers as thirty facts; they experience them as "server stuff goes in ServerScriptService" and absorb the rest over years.
...
| 3 | All three R15 refusals throw, with the named messages. **After `P7a` this pin is unchanged and must not be re-baselined**: the three sites read the counter's TOTAL lane, so a subtree of `BrushKind.Part` brushes is refused exactly as a world-brush subtree is. A **new** case is added when the relaxation ships — a `CanCollide = false` part brush under a `Server` node **succeeds**, and the same brush with `CanCollide = true` still throws, naming `CanCollide = false` as the route. |
