...
**Diagnostics, reserved codes `CV####`** mirroring `F4`'s `SS####` discipline. **The prefix was `SC####` and it moved, on 2026-09-02, when the cooker shipped.** `formats-and-pipeline.md` §4.1 claimed the same prefix, and the collision is resolved in the cooker's favour rather than by a coin toss, because the two claims are not symmetric: `scook` has bound `SC####` into the MSBuild-parseable stderr contract every consumer of a build tool parses, and into a band plan whose 6xxx entry **wraps** the shader compiler's `SS####` codes rather than renumbering them, so moving the cooker would break a code that is deliberately not the cooker's to renumber. Nothing in this arc is built, so this arc pays the move. Diagnostics: duplicate cvar name anywhere in the compilation; declaring type not partial; `Min > Max`; default outside `[Min, Max]`; name not matching `^[a-z][a-z0-9_]*$`; `Min`/`Max` on a non-numeric type; wrong handler signature; and **`Archive | Cheat` together as an error** — persisting a cheat cvar is a bug, not a configuration.
...
- **The `SC####` diagnostic prefix collision is RESOLVED (2026-09-02), in the cooker's favour**: this arc's generator uses `CV####`, and §4 records why the asymmetry decided it rather than a coin toss. The rest of this document is still specification rather than observation.
