...
Each of these is genuinely open, each blocks something concrete, and none is already decided elsewhere.
...
5. **Is multiplayer in the eventual plan, and on what model?** It decides whether `ScriptKind.Client`, `LocalScript` and a `RemoteEvent`/`Players` surface are real designs or placeholders — and it is arguably a larger data-model difference than everything else on the list combined. Reserving the enum value is cheap insurance but does not answer it.
