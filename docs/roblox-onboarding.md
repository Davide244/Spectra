...
- **It also gives `O7` a second, better answer for a spawned part.** A `BrushKind.Part` brush renders its own faces and moves at zero recompile cost, so *"parts created during Play are dynamic"* need not mean *"parts created during Play are `MeshRenderer` boxes"*. The `MeshRenderer` path stays right for an imported mesh; the part-brush path keeps planes, per-face materials and a hull. Which one `Instance.new("Part")` yields is still the §5 sign-off, and `physics.md` §2.3a explicitly declines to answer it — but the sign-off now has two viable answers rather than one. **The enum spelling that used to collide with this bullet is CLOSED (`physics.md` §7 item 10, decided by the user 2026-08-21): `BrushKind { World, Part }`, and `Part` naming two engine representations behind one Luau word is the DESIRED outcome, not a collision.** The distinction a scripter needs is World versus Part; which C# type backs a given part is an implementation detail that must not leak into the vocabulary. That *narrows* this sign-off rather than complicating it: whichever representation `Instance.new("Part")` yields, it is spelled `Part` either way, so the question is now purely about which one performs better for a spawned box — and the brush-backed path is the better default for exactly the reasons above.
