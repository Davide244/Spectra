...
**Amended 2026-08-21** to settle three of §9's open decisions — infrastructure (q1), P2P-versus-dedicated (q2) and whether shipped games need multiplayer (q4) — and to add ruling (5), the always-a-server model, with its topology matrix, shared-`Scene` mechanism and the §4.1 correction to the audience model's structural claim. Every source citation added in that amendment was read out of the tree the same day.

**Amended again 2026-08-21 — the container model is gone.** Where this document previously said *"replication scope is defined by WHERE a node lives"* and listed well-known top-level containers (`Workspace`, `ReplicatedStorage`, `ServerStorage`, `ServerScripts`, `ClientScripts`), the model is now **two inherited node properties** — `NodeRealm` (who holds it) and `NodeState` (is it live) — designed in full in **[`docs/realms.md`](realms.md)**, which is normative wherever this document and it disagree. **The axis name is locked: it is *realm*, decided by the user on 2026-08-21 (§9 item 12), so *scope* in this document means interest management and nothing else, permanently.** **Section and milestone numbering is unchanged** so existing cross-references keep resolving: §4.1 still carries the audience model and the local-client correction, and `N11` is still the milestone that lands it. What changed is only the *source* of the cached byte — from "the nearest well-known ancestor" to "the nearest explicit declaration" — plus three consequences this document owns and `realms.md` does not: the `.sentdef` `Flags` bit budget (§3.4), NetId numbering (§3.3), and `N11`'s scope (§8.2). **`realms.md` is design, not implementation; nothing in it exists in the tree.**
...
Authored map nodes derive their index from the `.scmap` `NODE` section's pre-order position, which `docs/formats-and-pipeline.md` §2.7 already pins to `SceneNode.Traverse()` order with `ParentIndex < SelfIndex` as an invariant. Both sides therefore agree on every authored node's id **with no negotiation and no spawn message** — the entire authored scene costs zero bytes of spawn traffic. Dynamic nodes allocate above `authoredNodeCount`.

**NetIds are allocated only to nodes whose effective realm is `Shared`, by ordinal in that same pre-order *restricted to `Shared` nodes*.** This is the rule `docs/realms.md` §8 states as a guarantee and its test pin 8 asserts, and it is stated here because this is the document that owns the numbering. Three consequences, each of which is a bug if the rule is written any other way:

- **A `Server`- or `Client`-realm node never gets a NetId at all**, because it never replicates: there is no baseline to keep, no delta to pack and no id to burn. That is what makes §4.1's containment structural on the wire rather than filtered at send time.
- **The numbering is identical across cook targets.** If a future `--target client` strip (`realms.md` §8) drops `Server`-realm records whole, an index derived from the *unrestricted* pre-order would renumber every node after the first stripped one and the two packs would disagree about what `NetId 41` means. Restricting the ordinal to the surviving `Shared` subset makes the client and server packs number identically by construction.
- **Per-player content is NOT a fourth audience.** `realms.md` §2.2 deletes `NodeRealm.Owner` for exactly this reason: content replicated to *one* client is by definition replicated, so it needs a wire identity, and a realm that is excluded from the numbering cannot have one. Roblox's `PlayerGui`/`Backpack` therefore map to **`Shared` nodes under a per-client interest filter** (§4.3's per-client `NetIdBitset`, plus `[Replicated(OwnerOnly = true)]` at property granularity in §3.4) — which is what Roblox does underneath and what this design already had. The two documents agree on this; neither carries it as an open question.

The residual edge, and it is genuinely open in both documents: a `Client`-realm node **created at runtime** would mean "instantiate this on every client" and has no NetId under this rule, so `realms.md` §9's carried-forward list refuses that creation in v1 — a node's realm is fixed at creation and it may only be created in a context that would hold it. That refusal has to be decided before the replication vocabulary is frozen, not after. NetIds within one snapshot are written ascending as 7-bit varint deltas, so a spatially coherent interest set typically costs one byte per object.
...
**Bits 3–5 are pinned here and may not be reassigned.** The full allocation of the `.sentdef` **keyvalue** record's `Flags` u32 — and of that record only — across the three documents that claim bits in it, so nobody has to reconstruct it. (The `.scmap` `NODE` record's `PayloadFlags` u16 is a separate table with a separate allocation, owned by `formats-and-pipeline.md` §2.7; realm sits at different bit positions there, and copying numbers between the two is a corruption bug.)

| bits | meaning | owner |
| --- | --- | --- |
| 0–2 | `readOnly`, `hideInEditor`, `requiresRestart` | `formats-and-pipeline.md` §3.2 (the record layout) |
| 3–5 | `replicated`, `unreliable`, `clientWritable` | **this section** |
| 6–7 | per-property realm, as one 2-bit value (`inherit` / `shared` / `server` / `client`) | `realms.md` §2.5, R13 |
| 8–31 | free | — |

`realms.md` takes 6–7 and **does not touch 3–5**; an earlier draft of that design proposed bits 3–4 for realm and bit 5 for `replicated`, which would have silently reassigned all three live bits. Both documents now agree on the table above. Two obligations follow: the record stays **fixed at 32 bytes** (a realm field is a bit pair, never a new member), and `D15`'s C#↔Luau byte-identity pin extends to bits 6–7 in the same change that lands them — the same way it already covers 3–5. `Entity.define`'s keyvalue descriptors gain the matching keys. `D15`'s parity pin extends unchanged: define the same entity twice, once in C# and once in Luau, and the two `.sentdef` records must be byte-identical apart from the one-byte Origin badge, **replication bits included**. `docs/formats-and-pipeline.md` §3.2 exists to prevent a second producer of this metadata; a `[NetProperty]` attribute with its own table would be exactly that.
...
2. **Every game pays a little authority indirection even solo** — a write goes through the owning side rather than straight into the node. The indirection is *not* a serialization cost; it is a call shape plus a compare against the executing VM's realm constant.
3. **Script authors think in server/client from the start.** For the Roblox-developer audience this arc targets, that is the familiar model rather than a tax — it is what `Script`/`LocalScript` and `ServerScriptService`/`StarterPlayerScripts` already mean to them, and `realms.md` §7 maps every one of those onto a declared realm.
...
**Replication audience is a declared, inherited property of the node — not a folder it was dragged into.** The full design is `docs/realms.md`; this paragraph is the part the wire needs and is deliberately not a summary of that document.

Two bytes per node, both defaulting to `Inherit` and both resolved down the tree:

- **`NodeRealm { Inherit, Shared, Server, Client }`** — who holds this node. `Shared` is the root's permanent value and every node's effective default, so the 99% of content that never thinks about audience declares nothing.
- **`NodeState { Inherit, Active, Dormant }`** — whether the node is *live*: carved, drawn, queried, ticking. This is the second axis and it is not optional; a parked template with no way to be inert would still carve the static world (`realms.md` §2.1 proves it against `Scene.SnapshotFullWalk` and `SceneBvh.IsSpatial`). **Ship both or ship neither.**

Resolution is an **intersection**, one line per axis, into a cached effective value maintained at exactly four sites — the two declaration setters, `AddChild` and `RemoveChild` — with `SetOwner`'s early-out. It is never attached to `MarkWorldDirty` or any transform setter. Two properties of the lattice matter to this document specifically:

- **Realm narrows going down and never widens.** A `Shared` child under a `Server` parent is *refused on an explicit write*, because the child's world matrix is `local * Parent.WorldMatrix` and a client that does not hold the parent could only resolve it by silently reparenting the child (geometry divergence with no error) or by ghost-replicating the hidden ancestor chain (leaking the thing the declaration existed to hide). Both are unacceptable, so the case never reaches the wire.
- **A reparent clamps by intersecting, and `Server ∩ Client` is EMPTY.** The empty intersection has a name — `RealmSet.Inert` — and the subtree replicates nowhere, runs nowhere and is badged in the Explorer. **It is never promoted to the ancestor's realm**, which would turn a dragged client HUD subtree into server code holding authority. Declarations are untouched by the clamp, so dragging the subtree back restores it exactly.

Where the model is written down: `.smap` carries `"realm"` and `"state"` as lowercase strings omitted when `Inherit`, written after `"name"` and before `"transform"` (`formats-and-pipeline.md` §2.6, which also owns the reserved-key list those two names join); `.scmap`'s `NODE` record carries the **declared** values in `PayloadFlags` bits 3–4 and 5–6 (`formats-and-pipeline.md` §2.7 — effective is rebuilt in the pre-order pass that already exists, since a runtime reparent must be able to recompute it); and `.sentdef` carries per-property realm in its keyvalue `Flags` bits 6–7 (§3.4). **The `.scmap` and `.sentdef` allocations are unrelated tables** and their bit numbers deliberately do not match; §3.4's table governs `.sentdef` only.

**This overturns two things this document previously asserted, and they must not survive anywhere:** that there is "deliberately no setter" (there is one, and it is legality-checked), and that the byte is resolved "from the nearest well-known ancestor" (it is resolved from the nearest explicit *declaration*). Everything §4.1 says below about the local client — the four leak channels, the accessor gate, the realm-aware resolver, `net_strictlocalclient` — survives unchanged; only the source of the byte moved.

**This overturned the `Storage`-collapse row in `docs/roblox-to-spectra.md`**, which mapped `ReplicatedStorage`/`ServerStorage` to "a single `Storage` node (deliberate difference)" on the grounds that "Spectra is single-process". That premise died the moment multiplayer became real. **That document's tree-and-services rows have been rewritten against the realm model** and must be kept in step with `realms.md` §7, which is the authoritative replacement table.

#### Correction: the audience model is NOT structural for the local client

This is the sharpest edge in ruling (5) and it was checked against source rather than accepted. An audience declaration claims that non-`Shared` content is "never sent, **not enumerable from a client**" — and without the enforcement below, that claim is false for the local client. For a *remote* client it is structurally true and needs no enforcement: the data never crosses the wire, so there is nothing to enumerate. For the **local** client — which shares the `Scene` — the node is physically present in the same graph, and every one of the following reaches it today with no realm concept anywhere in the path:

1. **The tree itself.** `Scene.Root` is public (`Scene.cs:60`), `SceneNode.Children` is a public `IReadOnlyList<SceneNode>` (`SceneNode.cs:75`), `SceneNode.Traverse()` is public (`:313`) and `Scene.Nodes => Root.Traverse()` is public (`Scene.cs:1392`). A client-side walk from the root reaches a `Server`-realm node in three hops.
2. **The id index.** `Scene.TryFindById` (`:186`) is a flat `Dictionary<Guid, SceneNode>` over **every** node in the graph, with no realm awareness. It is also the resolver every `IEditorCommand` and every inbound wire op goes through, so it is a leak channel in both directions: a client script that guesses or is handed a Guid resolves a server-realm node, and an inbound op naming one resolves it too.
3. **The spatial index.** `SceneBvh.IsSpatial(node) => node.MeshRenderer is not null || node.Brush is not null` (`SceneBvh.cs:146`) — ancestry-blind. So `Scene.Raycast` and `Scene.QueryFrustum` hit server-realm nodes, and `BuildRenderView` (`Scene.cs:269–285`) *draws* a `Server`-realm `MeshRenderer` on the local client's screen.
4. **The compiled world — and this one leaks to remote clients too.** `SnapshotFullWalk` iterates `Nodes`, i.e. the whole tree, and admits every node carrying a `Brush` (`Scene.cs:1345–1351`). A brush on a non-`Shared` node therefore *carves the static world*, and that compiled world is what every client renders and what `.scmap` ships. "Server-only geometry" is not server-only by any route. **This is why `realms.md` R15 refuses the combination outright rather than filtering it** — see the third bullet below.
5. **Counting oracles, which the first version of this list missed and which need no handle at all.** `Scene.NodeCount` is `public int NodeCount => _nodesById.Count` (`Scene.cs:193`) — an exact count of every node in the graph. `RenderView.TotalCount` is `view.TotalCount = Bvh.MeshLeafCount` (`Scene.cs:321`, backed by `SceneBvh._meshLeafCount`, `SceneBvh.cs:142`) — an exact count of every mesh leaf, hidden ones included. A client script reads either one before and after and detects server-side spawns without ever holding a node. `O3`'s planned `Scene.GetTagged(tag)` reverse index and `ObserveTag`, which *replays already-tagged nodes before connecting*, is the same bypass for a feature that is already designed rather than hypothetical.

Ruling (5) offers two mitigations for the "local client sees what a remote one never would" trap, neither of them a duplicate implementation: **(a) the audience model, sold as *structural* — a property of the node rather than a transport rule**, and **(b) a strict-local-client development cvar**. **So the honest statement is that (a) is structural for a remote client and, as specified, is a replication-time filter for a local one — which leaves (b) carrying the entire load in solo development, which is exactly the configuration a developer spends 95% of their time in.** That is the "works solo, breaks in multiplayer" trap wearing the costume of its own fix. The claim is repairable and cheap to repair, but it must be repaired inside `N11` rather than assumed.

**The fix: the realm check belongs in the accessor that issues node access to client-context code, not only in the replication filter.** Concretely, six changes, all inside `N11`'s realm byte, and all specified in full in `realms.md` §4 and §5:

- **Separate VMs first, then one gate at the handle boundary.** `O5` brings nodes into Luau as tagged userdata holding a `(slot, generation)` handle into a host-side table, and `O8` caches required modules by `SceneNode.Id`. On a *single shared* VM those two caches are the hole: a server script stashes a hidden node's userdata in a module table and a client script reads it back, and the gate never runs because the userdata already exists. So **one `lua_State`, one handle table and one `require` cache per net context** (`realms.md` R9), with the visibility mask a readonly field on the VM. That makes the transfer *impossible* rather than checked — and it is also what Roblox does, since a `ModuleScript` required from both sides yields separate instances with independent state. Then **`LuauHost.PushNode` pushes `nil` for a node not visible to the VM's mask** — one check in one function, and `O2`'s entire query surface (`FindFirstChild`, `GetChildren`, `GetDescendants`, `Parent`, `GetFullName`, `IsDescendantOf`, `WaitForChild`) inherits it without being patched individually. **A denial looks like absence, routed into each surface's existing absent path — never a new error kind** (`realms.md` R11): `FindFirstChild` returns nil, `GetChildren` omits, `GetDescendants` prunes the subtree, a property read falls to the generated `__index` switch's existing `default` (*"X is not a valid member of Y"*), a signal connection is skipped rather than invoked with nil. A bespoke "realm denied" error is an existence oracle under `pcall` and makes hosting behave differently from joining. Debuggability comes from a separate rate-limited dev-build diagnostic naming script, line, node and surface.
- **A realm-aware resolver beside the realm-blind one.** `Scene.TryFindById` stays as it is **by permanent contract** — the editor legitimately addresses every node in the graph, and that is the whole basis of `IEditorCommand` id addressing. Client-context script lookups and inbound wire ops go through a separately-named member that additionally checks the realm byte. The gate must bite **at the point of use**, not merely at the point of handout. Pin it with a test that resolves a `Server`-realm node's Guid from both, and asserts the client-context resolver fails.
- **Counts and indices are part of the gate, not an afterthought.** **Any scene-level index or count that spans hidden nodes is a bypass by default** — leak channel 5 above. The realm-aware forms take a mask (`Scene.VisibleNodeCount(mask)`, `Scene.GetTagged(tag, mask)`, `ObserveTag(tag, mask, …)`), the Luau binding layer must never name the blind form, and the rule generalises to any *new* public API returning or counting `SceneNode`s. Enforce it with a reflection-based test in the test assembly — reflection is legal there, and `Test/SpectraEngine.Editing.Tests/EditingAssemblyBoundaryTests.cs` is the existing precedent — or with a Roslyn analyzer.
- **A brush node is permanently `Shared`, refused at three O(1) sites, and the BVH stays realm-blind.** This replaces this document's earlier rule that "only `Workspace` contributes to the static world and to the BVH", which was written against containers and is wrong twice over. (i) **Admission is a hard refusal, never an exclusion-with-a-warning**: the `Brush` setter, the `Realm` setter (testing `SubtreeBrushCount > 0`, never `Brush is not null`) and `AddChild` all throw, so admission can *never* change by realm and the corruption class in the next bullet cannot exist for realm at all. The ground is the one this document already gave: a server-only brush would make the authority's `CsgWorld` differ from every client's, and `IServerAuthority.Validate` (§4.4) tests movement against exactly that world, so the player rubber-bands into a wall they cannot see. The refusal message must name `State = Dormant` **first** as the way to park a region and `MeshRenderer` second as the way to draw without carving. (ii) **`SceneBvh.IsSpatial` does not gain a realm condition** — one `Scene` serves both sides and a server script must still raycast server-realm content. It gains only the **liveness** condition (`node.IsLive && …`). The mask is threaded *through* BVH traversal and tested at **leaf-test time**, because a post-hoc filter of a nearest-hit raycast returns the wrong nearest hit. `Scene.Raycast`/`QueryFrustum` take a defaulted mask so every existing editor call site stays correct; **`BuildRenderView` takes it required**, so adding realm breaks every render call site exactly once and each is decided rather than defaulted.
- **Every admission change bumps `Scene._graphStructureVersion`, and `MarkStaticWorldDirty()` alone is not sufficient.** This is the hazard that makes the feature dangerous if implemented naively, and it belongs to `NodeState`, not to realm. `_graphStructureVersion` is bumped only by `OnNodeAdded`/`OnNodeRemoved`/`OnNodeSubtreeMoved` (`Scene.cs:128/137/159`) and its documented meaning is *"the brush snapshot's traversal ORDER may have changed"*; `MarkStaticWorldDirty()` sets only `_staticWorldVersion++` and `_snapshotForceFull` (`:724–728`); the compile launch trusts the carry when `_carryStructureVersion == _graphStructureVersion` (`:996`); and `CsgIncrementalCompiler.TryBuild` catches a pure count change (`:99`) but **not a count-preserving pair in one frame** — node A leaves the placement list because it went `Dormant` while node B gains a `Brush`. `VerifyTrustedDiff` is `[Conditional("DEBUG")]` (`:590–592`), so a dev build throws and a **Release build silently compiles corrupt geometry**. Add a named `Scene.MarkAdmissionChanged()` that bumps both, and pin it with a test that runs in the **Release** configuration.
...
**Ownership is explicit, not auto-assigned by proximity in v1.** Roblox auto-assigns *physics* ownership by proximity because it has a distributed physics solver to hand work to. Spectra had no physics engine when this section was written, so auto-assignment would have been a policy invented for a system that did not exist. **That is now a sequencing statement rather than a permanent one:** [`docs/physics.md`](physics.md) has since chosen an engine and designs the automatic policy in its §3.5 — a slow, hysteretic, server-side policy over `N13`'s interest grid (`sv_ownership_rate`/`_radius`/`_hysteresis`/`_mindwell`, `NetId` tiebreak, whole-joint-connected-set transfer) — landing in `Y12`, which depends on `N13`/`N16`/`N17`. Until `Y12`, ownership is set explicitly through the API below and nothing assigns it for you. Keeping the API *name* while deferring the policy is what let that arc slot in without renaming anything a developer wrote.

*(The `partial` keyword below is the same one-word change to `SceneNode`'s declaration that `realms.md` §2.4 needs — `SceneNode.cs:14` declares `public class SceneNode` and nothing in the tree is `partial` today. It appears here only to show the addition in isolation.)*
...
**Amended 2026-08-21, on two corrections `physics.md` §3.6 owed this section — both applied below.** (1) `NetInputCommand` **no longer carries `DeltaTime`**: under `N2`'s fixed tick it is redundant, and a client-supplied `dt` on a client-authored packet is free speed for anyone who edits it. (2) The purity contract is **"pure with respect to a pinned world revision"**, not pure full stop — a mover must query geometry, so literal purity was unsatisfiable by the only mover this design has.
...
{
    /// <summary>MUST be a pure function of (captured state, cmd) WITH RESPECT TO A
    /// PINNED WORLD REVISION: read-only queries against the world are permitted, and
    /// the design guarantees the queried world is identical between the original
    /// prediction and the replay. No other reads. The reconciler replays this N times
    /// in one frame from a restored state — which is why the fixed-tick loop is a hard
    /// prerequisite, not a later optimisation, and why dt is the tick, never a field
    /// on the packet.</summary>
...
The seam is expensive to retrofit; the simulation behind it is replaceable. Input sequencing, the client input ring, the `lastProcessedSequence` ack field on owned-instance updates and the compare-restore-replay driver all have to exist before any predicted subsystem does, and all three constrain the tick model. One concrete `IPredictedMover` ships: a kinematic character controller. **Since this section was written, [`docs/physics.md`](physics.md) has claimed that mover as its `Y5`** — it is Box3D's *stateless* character mover behind an `ICharacterCollisionSource`, whose collision source is the brush hulls rather than `CsgWorld.Raycast`/`ContainsPoint` directly (that arc's `Y4` demotes the CSG queries to the authored static world and makes physics the sole authority for gameplay answers). The seam, the sequencing and the deliverable are unchanged; only the body of the mover moved, which is precisely what "the simulation behind it is replaceable" was for. Two people walking around a level together is a genuine deliverable and exercises every seam under load. Where the mover is validated below (`Validate`'s default: a speed and teleport clamp plus a solidity test **with a skin width**, never bit-exact), `physics.md` §3.5 keeps the same envelope against hulls.

Rigid-body physics, physics rollback, proximity auto-ownership and lag compensation were **deferred by this document, and three of the four have since been designed elsewhere**: `physics.md` owns rigid bodies (`Y6`), automatic ownership (`Y12`, §3.5) and lag compensation (`Y16`, still deferred but explicitly **not** blocked on the engine choice), while **rollback stays deferred** behind that document's `Y13` re-evaluation gate — the chosen engine disclaims rollback determinism in writing, and nothing scheduled depends on predicted rigid bodies. Nothing here is retracted; what changed is that the deferrals now have an owner and an order. The per-instance state history ring remains **reserved and built** by this document, because entity interpolation needs the same ring and adding historical storage to every instance after gameplay exists changes every instance's memory layout.

`ApplyInput` purity is pinned by a test that replays the same input ring twice and asserts bit-identical end state. Any read of global state, `Random`, or the scene outside the mover's own node makes replay diverge from the original prediction, and the symptom is intermittent correction jitter that looks like a network problem. **Read the purity contract as amended above**: read-only world queries are permitted and are exactly what the pinned-revision guarantee covers; everything else on that list still diverges.
...
**Most "moving world" is not a world mutation at all.** A door, lift, platform, rotating fan or physics crate is a brush that **does not fuse into the compiled world**, so it replicates as an ordinary transform update with **zero geometry traffic and zero recompile on either side**. Stating this first dissolves roughly 95% of the question. The mechanism is `SceneNode.BrushKind` ([`physics.md`](physics.md) §2.3a, milestone `P7a`) for a part brush and `P7`'s nearest-`Entity`-ancestor ownership for an entity brush; both route through the same `IsStaticWorldBrush` predicate and the same non-carving render path.

> **Correction, same commit as `P7a`.** This paragraph previously said such brushes *"compile to entity-local `BrushModel`s attached as plain `MeshRenderer`s"*, and repeated `ROADMAP.md` `P7`'s claim of zero new render path. **That mechanism cannot be built:** `MeshRenderer` holds exactly one `Mesh` and one `Material` (`Scene/MeshRenderer.cs:12-20`) and a node holds one `MeshRenderer`, so after `F1`'s per-face materials it cannot express a multi-material brush at all. The replacement is one new arm in `BuildRenderView` plus one engine-owned refcounted mesh cache — **no third `RenderView` list and no backend change, but a new path, not zero new paths**. **Nothing in this section's network claims changes**: a non-carving brush still replicates as a transform, still carries no geometry traffic, and still costs no recompile. The dissolved 95% stays dissolved.
...
**`P7a` removes this trap for part brushes and leaves it fully intact for world brushes**, which is the distinction to keep straight. [`physics.md`](physics.md) §2.3a gates that exact line (`SceneNode.cs:140`) on `BrushKind`, so attaching, swapping or detaching a **`Part`** brush signals the static world nothing at all — spawning debris, projectiles or scripted props during Play costs **zero compiles on every peer**. A **`World`** brush attach or detach is unchanged and still forces the O(world) full walk on every client simultaneously, so the "replace, do not remove" rule above is still the rule for destructible world geometry. **Two residuals must not be forgotten:** until `P7a`'s optional structure-version narrowing lands (an explicitly separate step, with its own Release-configuration oracle), adding or removing a **part** node still bumps `_graphStructureVersion`, so parts spawning *while another peer's world-brush edit is in flight* still cost that peer one full walk; and `physics.md` §2.3a's gate 11 (`Scene.cs:1310`) must be implemented or a single part-brush descendant inside a dirty world-brush subtree forces the full walk every frame anyway.
...
```lua
-- Shop/Hud   -- node Realm = "Client". No LocalScript type; the NODE is marked.
local Remotes = workspace.Shop.Remotes          -- Shared: no container, just the tree
local buy     = Remotes.BuyItem                 -- RemoteEvent
local balance = Remotes.GetBalance              -- RemoteFunction

buy:FireServer("sword", 1)                             -- reliable, ordered
print(balance:InvokeServer())                          -- yields the coroutine

-- Shop/Till   -- node Realm = "Server", sitting RIGHT NEXT TO the remotes it serves.
local Remotes = workspace.Shop.Remotes
Remotes.BuyItem.OnServerEvent:Connect(function(player, itemId, count)
    print(player.Name .. " bought " .. count .. "x " .. itemId)   -- player is ALWAYS arg 1
end)

Remotes.GetBalance.OnServerInvoke = function(player)
    return Wallets[player.UserId] or 0
end

-- High-frequency, loss-tolerant: an UnreliableRemoteEvent payload instead.
Remotes.AimDirection:FireServer(camera.CFrame.LookVector)
```

Character-for-character Roblox from the first line of each handler down, including `RunService:IsServer()` and `Players.LocalPlayer`. **What is deliberately not character-for-character is the top of the file**: there is no `game:GetService("ReplicatedStorage")` to write, because there is no `ReplicatedStorage` — `Shared` is the default, so shared content is reached by walking the tree the game is organised as. The two scripts are siblings under one `Shop` node instead of being filed in two service folders, which is the whole point of `realms.md`. `game:GetService(name)` **throws**, naming the replacement (`realms.md` §7.4); it does not return `nil`, because the universal idiom binds the service on one line and indexes it on a later one, and a `nil` would report the wrong line. In C#, remotes are partial structs and the shared generator emits the codec and the `FireServer`/`OnServer` surface:
...
`RemoteEvent`/`UnreliableRemoteEvent`/`RemoteFunction` are node payloads whose channel ids are assigned from the deterministic node pre-order, so both sides agree with no negotiation.

> **OPEN — this collides with the payload-count rule and must be decided before `N15` is built.** `docs/data-model.md` §3 is the counting authority: `SceneNode` carries **two** payload fields today, four more are designed (`Entity`, `Script`, `PrefabInstance`, `Light`), and every addition is required to be a deliberate act on the hottest type in the engine — which is exactly why `docs/physics.md` §2.6 refused a body payload and used a flags byte plus a side table instead. Three more nullable reference fields for remotes would be the largest single addition anyone has proposed, made in passing. The three candidate shapes: **(a)** one `Remote` payload field carrying a kind enum (one field, not three); **(b)** no field at all — a per-scene side table keyed by `SceneNode.Id`, which is what `physics.md` does and what the deterministic pre-order channel numbering already implies; **(c)** three fields, accepted explicitly and recorded in `data-model.md`. Whichever wins, it is written down in `data-model.md` §3 in the same change. `RemoteFunction` re-entrancy is depth-capped and named in the error, the same way `P4`'s I/O cascade budget is.
...
7. **Realm containment** — the server parks a marker node declared `Realm = "Server"`; assert neither client's scene contains it, that a **client-context** resolve of its Guid fails on all three contexts (including the server's own, where the node *is* present — §4.1), that a client-context `GetChildren` over its parent omits it *in order*, that `Scene.VisibleNodeCount` under a client mask is unchanged by its existence (the counting oracle, leak channel 5), and that a `Dormant` brush subtree contributed **zero** placements and zero BVH leaves to any peer's compile. A brush on a non-`Shared` node cannot be constructed to test, because `realms.md` R15 refuses it at three setters — assert the refusal instead. This is the stage that turns §4.1's structural claim from prose into a gate, and it is the shortcut to `realms.md`'s master pin: **the local client's observation of a mixed tree must be identical, member for member, to the remote client's.**
...
20% loss in 7 ticks, Server-realm marker invisible to both clients,
...
- `O5` + `O8` (Luau bindings, `Script` payload, **one `lua_State` per net context**) — without them there is no client/server split consumer. Note the correction `realms.md` R11 owns: `O8`'s "reserve `ScriptKind.Client`" is **spent, not honoured**. `ScriptKind` loses its `Server` and `Client` members entirely and keeps only `bool IsModule`; the audience is the *node's* realm, so a client script is a node declared `Realm = "Client"` and there is no second script type. That correction has since landed in `roblox-onboarding.md` `O8`, which now specifies the single `IsModule` axis.
...
- `O5`'s node-handle bridge, as the single place a realm check can gate script access to a node (§4.1). Without it, `N11`'s audience model is a replication filter wearing a structural claim's clothes. And the bridge is only sufficient **if `O5`'s per-node handle cache and `O8`'s `require` cache are per net context rather than per scene** — a shared cache bypasses the gate at issue, because the handle was already minted for the other side.
...
| **N11** | Realms, liveness, and the scene-to-wire seam | **The design is `docs/realms.md`; this row is its wire-facing scope.** `NodeRealm` + `NodeState` on `SceneNode`, both `Inherit` by default; effective values as cached bytes resolved by **intersection**, with `RealmSet.Inert` for the empty one, maintained at exactly four sites (the two declaration setters, `AddChild`, `RemoveChild`) and never on a transform path; explicit writes legality-checked (narrowing only), reparents clamped without touching declarations; `.smap` `"realm"`/`"state"` strings (reserved keys, omitted when `Inherit`) and `.scmap` `NODE` **declared**-value payload bits; **NetIds allocated only to `Shared` nodes by ordinal in the `Shared`-restricted pre-order** (§3.3), asserted by a two-load test *and* by a client-target/server-target cook producing identical numbering; create/destroy/reparent on `GameReliable` in pre-order. **Plus the six §4.1 enforcement changes, which are the milestone's real content, not a footnote**: one `lua_State`/handle table/`require` cache per net context; the `PushNode` gate; a realm-aware id resolver beside the realm-blind `Scene.TryFindById`; mask-taking replacements for the counting oracles (`Scene.NodeCount`, `RenderView.TotalCount`, `O3`'s tag index); `SceneBvh.IsSpatial` gaining **`IsLive` only** with the realm mask threaded through traversal and tested at leaf-test time; and `Scene.MarkAdmissionChanged()` bumping `_graphStructureVersion` on every `State` transition over a brush-bearing subtree. **Keeps `roblox-to-spectra.md`'s tree-and-services rows in step in the same change** | N3, N4, `F2`, `O2`, **`O5`** (the handle boundary is where the gate lives), **`O8`** (the `require` cache must be per context, not per scene) | MEDIUM-**HIGH**, raised: it edits `SceneBvh`'s admission test and the render-view/query signatures, both Core hot paths, in addition to adding two bytes to the hottest type in the engine — consider packing `NetId` + dirty mask + realm/state bytes into one word, and **measure the layout rather than asserting the bytes land in padding**. Ruling `R‑9` applies: do not land concurrently with `E4`/`E6`/`P7`/`O2`. Two silent failure modes, both needing pins: a stale realm leaks `Server` content to the **local** client by direct tree walk rather than by a wire bug (pin with a 4×4 reparent matrix asserting the effective byte on every descendant, plus `N14` stage 7, run in the **published-binary** CI leg); and an admission change that does not bump `_graphStructureVersion` hands the incremental compiler a trusted carry against a shifted slot map, which `VerifyTrustedDiff` catches only in `DEBUG` — pin that one in the **Release** configuration | M–L |
...
| **N14** | Network self-test | The eight-stage in-process rig over a really-serializing loopback with injected loss/latency/reorder, explicitly seeded, **with `net_strictlocalclient` forced on** | N12 (six stages), N15 (the remote round trip), N11 (stage 7, realm containment) | LOW to build, highest leverage in the arc. Loopback hides every real-network failure mode, so injection is not optional — and stage 7 is what keeps §4.1's structural claim from decaying back into a comment | S–M |
| **N15** | Remotes, `Players`, the client script split | Three node payloads; the `[Remote]` generator surface; Luau bindings with `player`-first server signatures; `Players` as a **service and index, not a tree container** (a player has no transform, no brush and no realm; its contents — `player.Gui`, `player.Scripts`, `player.Character` — stay real `SceneNode`s in a per-player subtree, `Shared` under a per-client interest filter, `realms.md` §7.3); `player.Character` as a `NodeRef`; a script node declared `Realm = "Client"` as the whole of the "client script split" (there is no `ScriptKind.Client` to activate — `realms.md` §6.2 spends that reservation: `ScriptKind` loses both `Server` and `Client`, leaving only `Script.IsModule`); the per-VM realm constant so `IsServer` is context-scoped; the `Starter*` replacement as declarative **spawn rules** (`{ template, phase, destination }` in `game.spectraproj`, `realms.md` §7.2) rather than four magic containers; closed argument set with a loud call-site error naming the index | N11, N12, `O3`, `O5`, `O8` | MEDIUM. Two listen-host Luau contexts on one thread doubles `O8`'s registry-entry concern and turns `O9`'s single `lua_close` into two — cost it, do not assume, and note `realms.md` §9 Q5 has **not measured** the per-state `require`-cache memory. `StarterPack`'s destination does not exist: the spawn rule ports, tools/inventory is an unbuilt subsystem. **Server script source ships in the client's pack in v1** (`SCPT`/`LUAS` are one section): nothing in a server script is secret, and that must be documented before anyone puts a secret in one | M–L |
| **N16** | Ownership, input protocol, prediction, kinematic mover | `SetNetworkOwner`/`Auto`/`CanSet`; the permanent brush refusal with its error text; `IServerAuthority` with a **skin-width** validator; input sequencing, client ring, `lastProcessedSequence` ack, compare-restore-replay; one `IPredictedMover` — **which `physics.md` `Y5` now supplies** (the stateless character mover over brush hulls), so this milestone owns the prediction machinery and that one owns the mover's body | N2 (hard), N12, N13, `O7`; `Y5` for the mover itself | HIGH. Reconciliation looks fine on a LAN and rubber-bands on a real connection, so `N14`'s injection is mandatory here. Pin `ApplyInput` purity with a double-replay test. **Client-authoritative transforms are an exploit surface the moment they exist** — the validation envelope is mandatory at the descriptor level, pinned by a `(ownership × magnitude × scope)` matrix test as `C6` pins `(flag × ConVarSource)` | L |
...
- **`N11` is a correctness prerequisite for the local client, not only a wire feature** (§4.1), and it edits `SceneBvh`'s admission test, the query/render-view signatures and the structure-version contract the incremental compile trusts. That is Core surgery on the CSG hot path, so it obeys ruling `R‑9` and must be scheduled against `E4`/`E6`/`P7`/`O2` deliberately. It is also **not only a networking milestone** — `realms.md` is a scene-graph and data-model design first, so `N11` lands changes to `SceneNode`, `Scene`, the CSG admission predicate and the BVH whether or not a socket is ever bound.

Four edits to existing documents that must land with their milestones, not later:

1. **`ROADMAP.md` §12 (standing invariants)** gains five: *selection is presence, never an edit*; *`Scene.Camera` stays a `Camera`, never a `SceneNode`*; *no wall clock and no non-deterministic iteration order in the replication path* (adopting `formats-and-pipeline.md` §8's list verbatim); *there is no singleplayer code path — a solo game is a server with one local client sharing the `Scene` and no socket bound* (ruling (5)); and *at most one `Scene` per process may own a real `Renderer`* (§4.1, the constraint that keeps `net_strictlocalclient` and `N14`'s multi-scene rigs honest). Once `N11` lands, a sixth — **and note this is not the invariant an earlier draft of this document proposed** ("only `Workspace` contributes to the static world and to the BVH", which was written against the container model and is wrong on both halves): *a node admitted to the static-world carve has `EffectiveRealm == Shared` and `IsLive`, refused at the `Brush` setter, the `Realm` setter and `AddChild`; the BVH admits on `IsLive` and stays realm-blind, with realm filtered at the query boundary; and any change to which nodes are admitted bumps `Scene._graphStructureVersion`.*
...
3. **`docs/roblox-to-spectra.md`, tree-and-services rows**: the `Storage`-collapse row is overturned by `N11` and has been rewritten against the realm model, together with the `Workspace`, `Lighting`, `Players`/`StarterPlayer` and `LocalScript` rows. Those rows are **planned**, and the document must keep saying so — `realms.md` is design, nothing in it is in the tree.
4. **`docs/roblox-onboarding.md` `O8`**: *"`ScriptKind { Server, Client, Module }` … reserve Client so the naming never has to change"* is **spent, not honoured**. `ScriptKind` collapses to `bool IsModule`; audience is the node's realm. `O8` §5 q5 ("is multiplayer in the eventual plan") is answered by this document and by `realms.md`, and the "reserving the enum value is cheap insurance" line no longer describes what happens. **Corrected 2026-08-21**: `O8` now specifies `bool IsModule` and marks the reservation spent in place, and its §5 q5 is marked SETTLED naming this document and `realms.md` as the answer. Nothing further is owed here.

---

## 9. Decisions that need the user

Each is stated with its tradeoff in one sentence. None can be settled from the code. **Numbering is preserved across settlements** — four items are closed and stay in place rather than being removed, so cross-references from other documents keep resolving.
...
12. **SETTLED 2026-08-21 — the audience axis is called *realm*.** *(Was: is it called *realm* or *scope*?)* The user chose **`Realm`** over *scope*, on two grounds. *Realm* is Garry's Mod / Source Lua vocabulary for exactly this axis (`sv_`/`cl_`/`sh_`), so it arrives already meaning the right thing to the developers this arc targets; and **this document already uses *scope* for interest management and "replication scope" in adjacent paragraphs**, so shipping both words would put two meanings of one word into the two documents that have to be read together. The counter-argument — *realm* is a new word to learn, and *scope* is what the original ask said — was heard and overruled. The word is now frozen, because it goes into every `.smap` node record, every `.scmap` `NODE` payload-flag bit and every `.sentdef` `Flags` bit pair. **Binding consequence for this document: *scope* and *relevancy* keep their interest-management meaning permanently (§4.3), nothing here may use *scope* to mean node audience, and nothing may use *realm* to mean interest management.** Full record and the identifier list: `realms.md` §9 Q8.
...
- **Whether a realm gate at `O5`'s handle boundary is sufficient, given that C# game code shares the process with the authority.** The gate makes the audience model structural for *scripts* in every topology (§4.1). It cannot make it structural for **C#** on the local client, which can call `Scene.Root`, `Traverse()` or `TryFindById` directly — that is not a hole a runtime check can close in-process, and the honest answer is three-layered: a remote client genuinely does not have the data, `net_strictlocalclient` shows a solo developer the difference, and the C# surface gets a lint/analyzer rule rather than a runtime refusal. **Whether an analyzer rule is worth building, and what it keys on, is unresolved** — say "scripts", not "clients", in any documentation written before it is.
- **What one `lua_State` per net context costs in memory.** `realms.md` R9 requires separate states so a hidden node's userdata cannot be stashed across contexts, and R9 is not negotiable — a shared VM makes the gate decorative. But each state carries an independent `require` cache and therefore an independent instance of every `Shared` module's state, and **nobody has measured it**. Edit mode holds one state, Play holds a server state and a client state, plus a thread per script (`realms.md` §9 Q5). Measure before the listen-host topology is called shippable.
- **How a `Dormant` brush subtree going `Active` during Play interacts with `P7`'s dynamic-part split.** Both express "this exists but is not world geometry". If activation joins the carve it is a structural edit and a full recompile, so a spawner activating templates every few seconds would full-recompile the world on each one — the same hazard `P7` already names, and it must be answered together with it rather than separately (`realms.md` §9, carried forward).
