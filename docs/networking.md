...
**(4) Part of Roblox's ease is infrastructure, not engine.** Roblox hosts the servers and solves NAT traversal. Matching the *API* ease is an engine problem and is solvable here. Painless P2P across home routers is an operational commitment somebody must make. §7 says exactly which is which, and what it costs.
...
`docs/formats-and-pipeline.md` §3 makes the shipped runtime data-driven, so "the dedicated server is the same binary with a flag" is the right shape. The cost is smaller than expected on one axis and larger on another.
...
Two systems, one substrate, one transport. `SpectraEngine.Editing` continues to name no transport type — the same discipline that already keeps it free of Silk.NET and `IWindow`, enforced by extending `EditingAssemblyBoundaryTests` to ban `System.Net.*`.
...
A listen host is `Server | Client`; a dedicated host is `Server`; a joined player is `Client`. **`IsServer` and `IsClient` as read by a script report the calling context's side, not the process's topology** — implemented as a render-thread-scoped ambient `NetContext` set and cleared by the script/entity pump around each dispatch, keyed on `ScriptKind.Server` vs `ScriptKind.Client`. On a listen host both are true across a frame, which is exactly Roblox's `Enum.RunContext` model and exactly what makes ruling (1) true rather than aspirational. A process-level boolean would make every listen-server script answer wrong — the worst failure shape, because it is silent and only appears when someone first hosts.

**Replication scope is defined by WHERE a node lives, not by a flag.** Well-known top-level nodes under `Scene.Root`: `Workspace` (spatial, replicated, interest-filtered), `ReplicatedStorage` (non-spatial, replicated to all, client writes stay local and never travel back — Roblox's rule, verified and kept), `ServerStorage` / `ServerScripts` (never leave the server, not sent, not enumerable from a client), `ClientScripts` (replicated once at join, then run on the client). `ReplicatedFirst` is named and reserved.
...
**(d) Internet P2P.** `` ` `` → `host --public` prints `Join code: K7M-2QX`. Friend: `` ` `` → `connect K7M-2QX`. **This one is not free — see §7.**
...
7. **Restore** — undo, assert both clients returned bit-for-bit, disconnect both, assert the server's scene is exactly as found so the cycle repeats forever without drifting.
...
20% loss in 7 ticks, restored bit-for-bit.
```
...
### 7.4 The decision the user must make

This is the honest form of ruling (4), and it must be answered before `N6`'s documentation is written.

| Option | Cost to you | What the docs can claim |
|---|---|---|
...
**The recommended engine deliverable is (b) plus (d) as adapters**, with this sentence on the **first screen** of the multiplayer documentation, not in a footnote:

> `host --public` prints a six-character join code. Producing that code requires a rendezvous service, and roughly 10–30% of peer pairs will additionally need a relay. We ship both as self-hostable binaries and adapters for Steam Datagram Relay and Epic Online Services. **We do not host one for you.**

Shipping a "P2P" button that fails for a third of users, and explaining why afterwards, is a credibility loss the rest of the engine's honesty does not survive.
...
- **A verified NativeAOT publish of the transport** (`N0`), per the standing invariant.
...
| **N6** | Rendezvous, STUN, hole punching, relay client, honesty pass | `ISessionRendezvous` seam with a null impl for direct/LAN; RFC 8489 STUN client; drive LiteNetLib's `NatPunchModule` with retry and per-attempt timeout; self-hostable `spectra-rendezvous` and `spectra-relay`; SDR/EOS adapters; `net_connectivity`; **the §7.4 docs section** | N4 | MEDIUM technically; the real risk is that this is an **operational commitment, not a feature**. §7 | M + an open-ended operational tail |
...
| **N14** | Network self-test | The seven-stage in-process rig over a really-serializing loopback with injected loss/latency/reorder, explicitly seeded. Ship the six non-remote stages the moment `N12` lands | N12 (six stages), N15 (the seventh) | LOW to build, highest leverage in the arc. Loopback hides every real-network failure mode, so injection is not optional | S–M |
...
> **Networking** — `N0 → N1 → N2 → N3 → N4 → {N5, N10}` then `N11 → N12 → {N13, N14, N15} → N16 → N17`, with `N19`/`N20`/`N21`/`N22` off to the side. Needs `H1`-adjacent work only as an opportunity, not a gate.
...
1. **`ROADMAP.md` §12 (standing invariants)** gains three: *selection is presence, never an edit*; *`Scene.Camera` stays a `Camera`, never a `SceneNode`*; and *no wall clock and no non-deterministic iteration order in the replication path* (adopting `formats-and-pipeline.md` §8's list verbatim).
...
Each is stated with its tradeoff in one sentence. None can be settled from the code.

1. **Does anyone run a Spectra rendezvous and relay, and who pays?** Ship nothing (weakest claim, zero liability), ship self-hostable binaries (a €4.50/month VPS per operator, honest), operate a hosted default (matches Roblox's ease, permanent bill and abuse surface), or go Steam/EOS-only (free and excellent, forecloses non-Steam P2P). **This decides how `N6` is documented and must be answered before it is written.**
2. **Is P2P the headline shipping mode or the convenience mode?** If most games ship dedicated, `N6` leaves the critical path and the relay becomes a documentation note; if P2P is the headline, `N6` is load-bearing and somebody owns hosting.
...
4. **Do shipped games need multiplayer, or only the editor needs Team Edit?** If only the editor, `SpectraEngine.Net` never enters a shipped AOT binary and can be sized like `SpectraEngine.Editing`; if games need it, the AOT spike gates the whole arc.
...
- **Which transport survives the AOT spike.** LiteNetLib is the presumptive default on licence, dependency and API grounds; its NativeAOT posture is unverified and the standing invariant forbids assuming it. Blocks `N4`.
