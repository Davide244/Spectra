...
**Bits 3–5 are pinned here and may not be reassigned.** The full allocation of that `Flags` u32, across the three documents that claim bits in it, so nobody has to reconstruct it:
...
Where the model is written down: `.smap` carries `"realm"` and `"state"` as lowercase strings omitted when `Inherit`; `.scmap`'s `NODE` record carries the **declared** values in reserved `PayloadFlags` bits (effective is rebuilt in the pre-order pass that already exists, since a runtime reparent must be able to recompute it); `.sentdef` carries per-property realm in `Flags` bits 6–7 (§3.4).
