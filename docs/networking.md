...
12. **Is the audience axis called *realm* or *scope*?** (`realms.md` §9 Q8, restated here because this is the document with the competing usage.) **This document already uses *scope* for interest management and "replication scope" in adjacent paragraphs**, which is the strongest argument for *realm* — but *realm* is a new word to learn and *scope* is what was originally asked for. The word goes into every `.smap` node record, every `.scmap` `NODE` payload-flag bit and every `.sentdef` `Flags` bit pair, **so it cannot be changed after content exists.** Decide before the first map is saved. Whichever wins, the other must stop being used for the other concept in these documents, or the ambiguity survives the rename.
