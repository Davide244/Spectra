...
| `UnionOperation` / `NegateOperation` | CSG carve of the whole brush set | **deliberate difference** | Every brush carves every overlapping brush, continuously and non-destructively. There is no union *object* to create or bake. Note the carve is **union-skin extraction**, not subtraction: overlapping brushes *merge*, and a part resting on the floor does not punch a hole in it. **Planned qualifier:** once `BrushKind` ships ([`physics.md`](physics.md) §2.3a, `P7a`) this describes **world** brushes; a `Part` brush renders its own faces and never joins the fusion. |
...
- **True CSG.** Every brush carves every overlapping brush, live and non-destructively. No 5,000-triangle union cap, no baking, no negate-parts to manage. **Exists.** *(Planned: a brush may opt out of the fusion by declaring `BrushKind = Part` — `physics.md` §2.3a. Opting out is a declaration, never a side effect of moving, parenting or simulating something.)*
