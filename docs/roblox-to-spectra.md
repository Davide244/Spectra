...
| `Anchored` | `Anchored = true` is what a brush already is; `false` is a **conversion** | **planned** | **Corrected — an earlier version of this row said `false` would throw.** [`docs/physics.md`](physics.md) §4.2 overruled that: throwing takes away the parts-are-placed-freely pillar to protect an implementation detail. At edit time, setting `Anchored = false` on a brush node is an undoable editor command that swaps it for a dynamic part (`O7`); at run time it is a physics body. Brushes still compile into the fused world by construction, which is why the change is a conversion rather than a flag. |
| `CanCollide` / `CanQuery` | bits in the `PhysicsFlags` byte on the node | **planned** | `docs/physics.md` `Y0` owns them, along with the collision-group registry and the `SceneBvh` box/sphere overlap queries that do not exist yet. One named edge: `CanQuery = false` cannot be honoured on a *static world brush* (it is derived from the carve, and excluding it would change compiled output), so it is refused there with a message and is legal on dynamic parts. |
