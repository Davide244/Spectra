...
- ~~**Whether `CanQuery = false` should be legal on a part brush but refused on a world brush**~~ — **CLOSED 2026-08-21: it is legal and honoured on both**, because the refusal's premise (that a world brush's queries route through the per-cell BSP) is false for `Scene.Raycast`, which walks the spatial index per node. See §7 item 11.
...
| --- | --- | --- | --- | --- | --- |
| **Y0** | Query flags and BVH overlap queries | `PhysicsFlags` byte on `SceneNode` with the brush-node `Anchored` conversion path (**which is now `ConvertBrushKindCommand`**, §2.3a / §4.2); `QueryParams` with `RespectCanCollide`; **`SceneBvh` gains box and sphere overlap queries** (it has only `Raycast` and `QueryFrustum` today) — **`P8`'s trigger queries now depend on this**, because §2.3a killed the brush-model BSP they were specified against; `Scene.GetPartBoundsInBox`/`InRadius`; `Scene.Raycast` starts honouring `CanQuery`; the 64-group `CollisionGroups` registry. **No native code, no physics engine.** | `F2`; `P7a` for `BrushKind` | **SHIPPED 2026-08-21** (commit `611bb22`, plus the adversarial-review repair that follows it). ~~One sharp edge: refuse `CanQuery = false` on `BrushKind.World` brushes~~ — **OVERTURNED IN IMPLEMENTATION.** The refusal rested on the premise that a world brush's queries route through the per-cell BSP; `Scene.Raycast` walks the **spatial index per node**, so the flag is one bit test there and is honoured for every kind. Only `CsgWorld.Raycast`/`ContainsPoint` cannot honour it, and they are demoted in their own XML docs for that exact reason. See §7 item 11 for the full argument, including the ordering trap a refusal would have created. Also shipped beyond the row's wording: subtractive brushes are skipped by every query except editor picking (a hole contributes no solid, so a shot at a doorway must not stop in mid-air), `SceneQueryFilter` carries the ignore list and group filter, `QueryFrustum` stays deliberately flag-blind (visibility is not a gameplay question), and `Scene.Raycast`'s XML doc now states plainly that it is **authored-geometry authority, not a gameplay query** — it reports solid inside a carved doorway, and that divergence unifies only at `Y4`. `CanCollide = false` on a world brush **is** honoured, for free, because hulls come from placements. `Y0` is also where the `realms.md` R15 relaxation becomes *reachable* — it ships with whichever of `Y0`/`Y8` first makes `CanCollide` real, never with `P7a` | S–M |
...
| **Y5** ✅ | Character mover — the one `IPredictedMover` | `ICharacterCollisionSource` plus the brush-hull implementation; sweep-and-slide, step probe, ground snap, slope limit, air control, jump, `CharacterState`; `CaptureState`/`RestoreState` as a struct copy; the kinematic **presence body**; `PredictionContext` replacing the bare `dt`. **This is `N16`'s mover** | `Y0`, `N2` (hard), `Y3`/`Y4` for the backend source | **SHIPPED 2026-08-22**, with three scope changes and three defects worth recording. **(1) One collision source, not two.** The row scheduled a Box3D-backed second source plus a dual-source agreement test; the plane-set source needs no native dependency at all — which is also what a dedicated server wants — so the second source and its five P/Invokes were cut on the design review's own finding, and sign-off §7 item 5 is answered by implementation. **(2) The first-person host layer landed with it** (`FirstPersonController`, F8/`--play`, cursor lock, fall-out respawn, capsule overlay on F9), plus `ISceneEditor.Suspend()` — entering play mode is a transfer of the camera AND the cursor away from an editor that may be holding both mid-gesture, and doing it in one place is what stops either being owned twice. **(3) `DemoPlayArea`, a human-scale course**, because the demo room is 6x6 with a 1.2-unit doorway and could not be walked at all. Defects the *content* found that no unit test could: `StepHeight` was not the number it claimed — a capsule's rounded bottom lets it come to rest against the top edge of a taller riser inside the slope limit, so a documented 0.45 climbed 0.50 risers, now bounded by the landing CONTACT POINT's height; the world lane invalidated on the compile counter, so a scene where anything animates rebuilt the character's whole neighbourhood 60 times a second (measured 1,498 rebuilds in 25 s, now 1); and coincident contact planes from overlapping brushes — a staircase is boxes sunk into a floor — spent the 8-plane contact budget on copies of one surface (measured 101 dropped planes in 10 s of walking, now 0). **Still open and NOT claimed:** feel. Acceleration, friction and air control are chosen numbers and only a human hand finds jitter or catching on seams. The `AirSpeedCap = 1.0` default in particular makes any partial-depth pit against a vertical wall a trap the respawn must undo, which is why the course's chasm is cut clean through | L |
...
1. **~~What is one Spectra world unit in metres?~~ CLOSED by the user on 2026-08-21. One world unit is ONE METRE, and the unit has a name: the *spectraunit*, abbreviated `sunit`.** So gravity is `9.81 sunit/s²`, a 32-unit chunk is 32 m, a door is ~2 sunits tall, and `b3SetLengthUnitsPerMeter(1.0f)` is the startup call — i.e. the solver runs in exactly the range it was tuned for, with no rescaling of its tolerance macros. **The cost is real and is accepted rather than hidden:** a ported Roblox place must scale, because Roblox is internally inconsistent about its own stud (the Creator Hub documents 0.28 m/stud while its default gravity of 196.2 studs/s² against 9.81 m/s² implies 0.05 m/stud — a factor of 5.6 between two official numbers). Porting therefore cannot have been mechanical under *any* choice, which is what makes readability the deciding criterion: the alternative bought a fake mechanical port and paid for it with a solver at an untuned scale and a 9 m chunk. **Consequences to land with the answer:** `roblox-to-spectra.md`'s `1 stud` row states the conversion the porter applies, `ROADMAP.md` §11.13 is answered, and `PhysicsDefaults` states gravity, sleep threshold and density in sunits in one file.
2. **~~Float or double Box3D build?~~ CLOSED by the user on 2026-08-21. FLOAT**, i.e. `BOX3D_DOUBLE_PRECISION` off. The per-chunk-body design already closes the static world's precision exposure at any distance, so this bounds *dynamic* bodies only: roughly 1 mm at 8 km — the distance `CsgBench openworld` actually exercises — and roughly 1.6 cm at 100 km. **The decision is load-bearing on the binding's first line, not on a build flag later:** the flag is ABI-affecting (the two modes ship as mutually exclusive packages for that reason), so `B3Pos` is still written as a type distinct from `B3Vec3` from the first line of generated code, which is what keeps a future double build a re-generation rather than a rewrite.
...
11. **~~Is `CanQuery = false` legal on a part brush while refused on a world brush?~~ CLOSED 2026-08-21 by implementation, and the question's own premise was false.** `Y0` shipped honouring `CanQuery` on **every** kind of node. The refusal was specified because a world brush's queries were believed to route through the compiled per-cell BSP, which cannot exclude a single brush — the BSP is derived from the carve, and excluding one would change compiled output the determinism oracles compare. **That premise does not hold for the entry point in question:** `Scene.Raycast` traverses the spatial index **per node** (`SceneBvh.Raycast`), where the flag is one bit test on a cache line already being read. What genuinely cannot honour it is `CsgWorld.Raycast`/`ContainsPoint` — which are demoted for exactly that reason, and now say so in their own XML docs. The honest rule is therefore one sentence about *which query you called*, not a refusal on a setter. **A refusal would also have built an ordering trap with no good exit:** clear `CanQuery` on a part brush, then convert it to world geometry, and the refusal must either reject the conversion — making conversion a refusal site, which is precisely what `BrushKind` exists to avoid — or silently rewrite the author's flag. Honouring it everywhere removes the asymmetry this sign-off was weighing, which is the outcome its own second option preferred.
...
**~~Everything in this document is design. Nothing was built, run, profiled or measured.~~ NO LONGER TRUE as of 2026-08-22.** `Y0` shipped, the `Y1` native gate shipped (Box3D vendored as a pinned submodule, built, bound, and proven loadable under `[LibraryImport]`), `Y2`'s seam and world lifecycle shipped, and `Y3`'s static collision shipped: the demo compiles 198 authored brushes into 78 per-chunk static bodies carrying 197 convex hulls, and a dynamic body falls and rests on them at the correct height. What follows is still design **except** where a milestone row below says otherwise. Facts corrected by contact with the real library are marked in place — three of this document's own claims were wrong, listed at the end of this section. The frame placement, seams and API sketches are source reads of this repository on 2026-08-21; the Box3D facts are primary-source fetches on the same date, listed with their URLs in §1.1 so they can be re-checked rather than trusted.

**Three claims this document made that the implementation disproved:** (1) `b3SetLengthUnitsPerMeter` was described as needing to precede the first *world* — it must precede the first `b3Default*Def()`, which bakes the scale in at call time. (2) Density was described as a quantity that does not rescale with the length unit — it rescales by the inverse cube. (3) The hull limits were taken to be 128 vertices and 128 faces independently; the check that actually binds is on **edges**, so Euler gives `V + F ≤ 130` and a 128-vertex brush is impossible. All three are corrected where they appear.

**One thing the implementation confirmed by experiment rather than reading:** attaching a hull to a shape *copies* it, so hulls may be freed as soon as their shapes exist. That decided the static-sync cache's lifetime and removed a refcounting scheme the design had assumed.
