...
- **Deliberately out of scope:** `TreeDataGrid` virtualization (the ceiling is ~250 realized rows and the demo scene is 257, so this is the next thing to hit), reveal-and-expand on viewport selection, drag-reparent, rename, tree keyboard multi-select, layout persistence, and the radial background glow (it cannot appear behind the viewport, and it bands on 8-bit panels).
