...
### E9 — Switchable gizmo styles: Studio and Classic *(specified by the 2026-08-27 gizmo audit; the behavioral fixes it ordered are already landed)*
The current gizmo is the classic Blender/Unity layout (single-ended arrows + plane quads + centre disc; rings; tip cubes) with corrected drag math. This milestone adds the Roblox-Studio style and the seam to switch natively. **Landed ahead of it:** delta snapping as the default with `TranslateSnapMode.AbsoluteGrid` opt-in anchored on the reference node, screen-basis free-drag snapping, NaN-refusing parallel guards plus the mid-drag zero-viewport hold, and pick/drag agreement on translate/scale axis handles (`GizmoRobustnessTests`, `GizmoSnappingTests`).
- **The seam:** an `IGizmoStyle` strategy resolved at tool construction (allocation-free per frame): a proportions record replacing `GizmoGeometry`'s private consts (default reproduces today's values bit-for-bit), `Pick` (contract: never offer a handle `TryPrepareDrag` would refuse), `Draw`, per-handle constraint mapping, and policy data (free-move mapping, pivot mode, resize face anchoring, multi-select resize policy, modifier assignments, palette). `GizmoController.Style` mirrors `Orientation` exactly (equality early-out, `Active.Reset()` before the switch, `GizmoCommand.ToggleStyle` verb, name-resolved shortcut). The `GizmoTool` state machine, transactions, capture, id-addressed absolute commands, snap ladders, and the no-Silk.NET rule do not vary.
- **Studio style content:** six per-direction cone arrows (needs `GizmoHandle` negative-axis values; every existing switch degrades safely on unknown values), six per-face resize handles (negative faces anchor the opposite face: pass the local-bounds MAX and negate travel — `ResizeMath.AnchorShift` already takes any anchor), surface dragging for press-on-object (per-frame scene raycast excluding the drag targets via an `IGizmoSurfaceQuery` backed by the scene the tool already holds, with the last surface extended as a virtual plane when the ray leaves geometry), selection-bounds pivot for multi-select, and group-box multi-select resize (sizes and offsets scale together about the box centre). Classic keeps camera-plane free move and per-node resize.
- **Also owed here:** a filled-triangle batch in `DebugDraw` (same depth-off overlay pass) so plane quads, the centre disc and the centre cube can fill the interiors they pick — every mainstream editor tints them, and Studio's snap tick markers want the same primitive.
- **Depends on** — `E1`, `E4`, `E5` (all landed). **Risk** — **MEDIUM**: the surface-drag constraint is a new kind and must refuse on the same sin² scale as the existing guards; shape-pinning tests parameterize per style while invariant suites (cancel exactness, held-click, allocation-free steady state) run under BOTH styles.
- **Size** — **M–L.**
