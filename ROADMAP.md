...
- **Why the geometry pass was NOT done with it.** An instanced shader is a second shader, and SpectraShade's `import` is parsed but never resolved (no resolver exists), so sharing code between the two is not currently possible. Duplicating `ShadowDepth` duplicates an empty fragment stage; duplicating `GBufferFill` would duplicate the five-attachment write that `CLAUDE.md` already names as a drift hazard. **Resolve imports first, or make every draw instanced so one shader serves both** - the second is the cleaner endpoint and removes `uModel` as a uniform entirely.
