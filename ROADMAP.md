...
**R‑9. `P7` (entity brush ownership) must not land concurrently with `E4`/`E6`.**
Both do surgery on `SceneNode`'s counters and the brush snapshot path. `P7` is the single riskiest edit in the roadmap; give it a quiet tree.
...
- **Entities & no-code logic** — `P4 → P5 → P6 → P7 → P8 → P9 → P10`. Needs `F2`; `P9` needs `P2`.
...
### P7 — Brush entities: excluded from the carve, compiled as local-space brush models
**The keystone, and the highest-risk milestone in the roadmap.** A brush is owned by its **nearest `Entity` ancestor, inclusive** (`_entityDepth` counter mirroring the existing `_subtreeBrushCount` idiom); `_subtreeBrushCount` **splits** into a static-world-only counter; the snapshot filters on `IsStaticWorldBrush`. Entity-owned brushes compile in **entity-local space** through the existing **monolithic** carve → snap → weld → `CsgWorld.BuildMeshArrays` path into a `BrushModel`, attached as a plain `MeshRenderer` on the entity node — **zero renderer changes, zero backend changes, zero new `RenderView` path**, and a door opening becomes a matrix write with no recompile.
- **Named hazards, all three of which are silent:** (1) forgetting the counter split makes an animating door mark the world dirty and launch a background compile **every frame, forever** — the single most likely way to destroy the pillar while everything still renders correctly; (2) an ownership flip that does not force a full walk corrupts the slot map, because the placement *count* changes and every later slot shifts; (3) an ownership flip that does not dirty the departing brush's footprint leaves stale geometry welded into the world.
- **Required verification — this must not merge on code review alone:** all chunked-vs-monolithic and determinism oracles green; an entity-owned brush contributes zero world surfaces; `StaticWorldCompileCount` **constant** across 100 frames of door animation; brush-model triangles identical to `CsgWorld.Build(sameplacements).BuildMesh()` at identity; `CsgBench openworld` verdict still *world-size independent*.
- **Depends on** — `P4`. Independent of `P5`/`P6`. **Must not run concurrently with `E4`/`E6`** (ruling R‑9).
- **Size** — **L** (modest line count, extreme care density).

### P8 — Trigger volumes and brush-model queries
`trigger_multiple`/`trigger_once`/`trigger_teleport`; queries transform into model space via the inverse of the entity node's world matrix and hit the brush model's BSP. Touch tracking diffed per tick drives `OnStartTouch`/`OnEndTouch`/`OnTrigger`.
...
- The `CsgBench openworld` verdict line must keep saying **world-size independent**. Any milestone touching the snapshot, slot map, footprint diff or trusted-diff contract (`F1`, `E4`, `E6`, `P7`) must show it, not assume it.
...
- **Should a brush model build its BSP eagerly or lazily?** Eager costs build time and memory for every decorative part; lazy means the first collision query on a moving platform pays an unpredictable spike. Leaning lazy with explicit opt-in for triggers and solids.
