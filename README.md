...
| **Rendering** | OpenGL, D3D11 and D3D12 backends, live. Forward and wireframe pipelines over one shared draw list. All shading in linear light, with sRGB decode and display encode done by the hardware. |
...
- **`R3` offscreen render targets is the rendering keystone.** Shadows, post
  processing, anti-aliasing, material previews and the Uno viewport all wait on
  it, and `R1` and `R2` were its prerequisites. Both have landed.
...
| **R** Rendering | [ROADMAP](ROADMAP.md) | R1 PSO key, R2 sRGB | R3 render targets |
